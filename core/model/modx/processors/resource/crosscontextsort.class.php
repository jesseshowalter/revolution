 ...
 */
class modResourceCrossContextSortProcessor extends modProcessor {
    public $nodes = array();
    ...
    public function process() {
        ...
        $target_context = $this->getProperty('context');
        ...
        foreach ($this->nodes as $ar_node) {
            ...
            if ($old_context_key != $ar_node['context'] && !empty($ar_node['context'])) {
                $node->set('context_key',$target_context);
    ...
    protected function getNodesFormatted($currentLevel,$parent = 0) {
        ...
        foreach ($currentLevel as $id => $children) {
            ...
            if ($explodedArray[1] != '0') {
                ...
                $parentId = count($explodedParentArray)>1? $explodedParentArray[1] : 0;
                $this->nodes[] = array(
                    'id' => $explodedArray[1],
                    'context' => $this->target_context,
                    'parent' => $parentId,
                    'order' => $order,
                );
                $order++;
...
return 'modResourceCrossContextSortProcessor';
